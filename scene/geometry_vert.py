...
def draw_walls(graph, map, bm):
    ...
    for sector_id, linedefs in graph.items():
        ...
        for linedef_data in linedefs:
            v1_coords = (
                vertices[linedef_data["linedef"][0]].x,
                vertices[linedef_data["linedef"][0]].y,
            )
            v2_coords = (
                vertices[linedef_data["linedef"][1]].x,
                vertices[linedef_data["linedef"][1]].y,
            )
            ...
            if not linedef_data["twosided"]:
                add_wall(
                    v1_coords,
                    v2_coords,
                    current_sector.heightfloor,
                    current_sector.heightceiling,
                    bm,
                    face_metadata,
                    linedef_data["front_sidedef"],
                    walltype="MIDDLE",
                )
            else:
                # For two-sided walls
                ...
                if current_sector.heightceiling != other_sector.heightceiling:
                    upper_start = min(
                        current_sector.heightceiling, other_sector.heightceiling
                    )
                    upper_end = max(
                        current_sector.heightceiling, other_sector.heightceiling
                    )
                ...
                if current_sector.heightfloor != other_sector.heightfloor:
                    lower_start = min(
                        current_sector.heightfloor, other_sector.heightfloor
                    )
                    lower_end = max(
                        current_sector.heightfloor, other_sector.heightfloor
                    )
